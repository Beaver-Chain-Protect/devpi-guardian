"""Resolve, verify, quarantine, and register durable discovery jobs."""
...
class SimpleLinkResolver:
    """Validate a Simple link and derive F4's source-stage release identity."""
    ...
    def resolve(self, candidate: DiscoveryCandidate) -> ResolvedDiscovery:
        ...
        origin = (parsed.scheme.lower(), parsed.hostname, parsed.port)
        ...
        if parsed.query:
            raise ValueError("artifact link query parameters are not accepted")

        parts = PurePosixPath(parsed.path).parts
        if parts and parts[0] == "/":
            parts = parts[1:]
        if len(parts) < 5 or parts[2] not in ("+f", "+e"):
            raise ValueError("artifact link is not a devpi file path")
        if parts[-1] != candidate.filename:
            raise ValueError("artifact filename does not match link path")

        fragment_sha = parse_qs(parsed.fragment).get("sha256")
        if fragment_sha is not None and fragment_sha != [candidate.sha256]:
...
class DiscoveryConsumer:
    """Process at most one metadata job and ACK only after F4 commits."""
        ...
        try:
            ...
            if verified is None:
                verified = self._quarantine.persist(
                    candidate,
                    self._bytes_source.iter_chunks(resolved),
                )
