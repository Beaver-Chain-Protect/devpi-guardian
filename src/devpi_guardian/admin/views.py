...
from .service import AdminMutationsUnavailable
...
def configure_admin_routes(pyramid_config) -> None:
    ...
    routes = (
        ("guardian_quarantine", f"{prefix}/quarantine", list_quarantine, "GET"),
        ("guardian_artifact", f"{prefix}/artifacts/{{sha256}}", inspect_artifact, "GET"),
        (
            "guardian_artifact_approve",
            f"{prefix}/artifacts/{{sha256}}/approve",
            approve_artifact,
            "POST",
        ),
        (
            "guardian_artifact_block",
            f"{prefix}/artifacts/{{sha256}}/block",
            block_artifact,
            "POST",
        ),
        (
            "guardian_artifact_rescan",
            f"{prefix}/artifacts/{{sha256}}/rescan",
            rescan_artifact,
            "POST",
        ),
        (
        ...
        ("guardian_health", f"{prefix}/health", health, "GET"),
    )
    for name, path, view, method in routes:
