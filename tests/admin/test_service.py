...
class Reader:
    ...
    def health(self):
        return {"database": "ok", "schema_version": 6}
...
class Store:
    ...
    def revoke_manual_override(self, sha256, actor, reason):
        self.revocation = (sha256, actor, reason)
...
class Operations:
    ...
    def worker_health(self):
        return {"status": "running", "pending_jobs": 2, "processing_jobs": 1, "failed_jobs": 0}
...
def test_service_lists_quarantine_and_inspects_by_sha256() -> None:
    ...
    page = service.list_quarantine(states=(ArtifactState.REVIEW,), limit=25, offset=10)
...
def test_service_approve_block_exception_and_rescan_use_f4_transitions() -> None:
    ...
    service.add_exception(
        SHA256, actor="root", reason="temporary build unblock", expires_at=expires_at
    )
...
def test_service_revoke_propagates_authenticated_actor_and_reason() -> None:
    store = Store()
    service = GuardianAdminService(reader=Reader(), store=store, now=lambda: NOW)

    service.revoke(SHA256, actor="root", reason="review withdrawn")

    assert store.revocation == (SHA256, "root", "review withdrawn")
...
def test_service_delegates_remaining_f11_operations() -> None:
    ...
    assert (
        service.list_audit(
            sha256=SHA256, actor="root", action="artifact.approve", limit=20, offset=0
        )["total"]
        == 1
    )
...
def test_service_fails_closed_when_optional_operation_provider_is_missing() -> None:
    ...
    operations = (
        ...
        lambda: service.list_audit(sha256=None, actor=None, action=None, limit=20, offset=0),
